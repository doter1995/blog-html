...
window.onload = function () {
  var width = d3.select ('#root').style ('background-color', '#eee').node ()
    .clientWidth;
  ...
  var draged = d3
    ...
    .on ('drag', function (d, i) {
      ...
      d.dx += d3.event.dx;
      d.dy += d3.event.dy;
      var node = d3.select (this).attr ('transform', getTransform);
    ...
    .on ('end', function (d, i) {
      ...
      var targ =
        parseInt (active) + parseInt (moveRight) + parseInt (moveDown) * 3; //获取到移动的目标点
      if (targ > -1 && targ < 9 && targ != active) {
        //移动
        var targ1, active1;
